...
df = complex_dfs['objectives']
st.title("Visualisation des objectifs par thématique")

# Compter le nombre de fois que chaque label apparaît
topic_counts = df['objectives_topic_label'].value_counts().reset_index()
topic_counts.columns = ['objectives_topic_label', 'count']

# Création du pie chart avec Plotly
fig = px.pie(
    topic_counts,
    names='objectives_topic_label',
    values='count',
...
selected_topic = st.selectbox(
    ...
    topic_counts['objectives_topic_label'].unique()
)

# --- FILTRAGE ---
filtered_df = df[df['objectives_topic_label'] == selected_topic]

# --- WORD CLOUD ---
st.subheader(f"Nuage de mots pour : {selected_topic}")

# Fusionner tous les textes en un seul corpus
text = " ".join(filtered_df['objectives_topic_representation'].dropna().astype(str))
...
st.subheader("Objectifs associés")
st.dataframe(filtered_df[['id', 'objectives_topic_representation']])

#----- shows entities --------------
# Sélection via clic simulé : on utilise un selectbox comme alternative simple
st.subheader("Sélectionne une thématique pour afficher les objectifs associés")
selected_topic = st.selectbox("Choisis une thématique :", topic_counts['objectives_topic_label'].unique())

# Filtrer les IDs associés à ce label
selected_ids = df[df['objectives_topic_label'] == selected_topic]['id']
